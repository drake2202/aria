...
from .config import PEPPER_FLASH_SO, account_paths, account_key

log = logging.getLogger("gameview")


def configure_flash_flags():
    """
    Inject Chromium command-line flags for PepperFlash PPAPI.

    MUST be called BEFORE QApplication is created — Qt5 WebEngine reads
    argv and env at QApplication construction time.
    """
    flash_path = str(PEPPER_FLASH_SO)
    if not PEPPER_FLASH_SO.exists():
        log.warning("PepperFlash not found at %s", flash_path)
        return

    # Force scale factor to 1 — Flash SWFs render text oversized with HiDPI scaling
    os.environ["QT_AUTO_SCREEN_SCALE_FACTOR"] = "0"
    os.environ["QT_SCALE_FACTOR"] = "1"
    os.environ.setdefault("QT_SCREEN_SCALE_FACTORS", "1")

    # Method 1: sys.argv flags (most reliable for Qt5 WebEngine)
    flash_flags = [
        ...
        # Performance: GPU compositing and reduced overhead
        "--enable-gpu-rasterization",
        "--enable-zero-copy",
        "--ignore-gpu-blocklist",
        "--disable-gpu-driver-bug-workarounds",
        "--enable-native-gpu-memory-buffers",
        # Allow multi-process to prevent crashes during cross-origin navigation
    ...
    # Method 2: env var (backup)
    os.environ["QTWEBENGINE_CHROMIUM_FLAGS"] = " ".join(flash_flags)

    log.info("Configured PepperFlash PPAPI: %s", flash_path)
